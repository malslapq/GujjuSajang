...
public class StockService {
    ...
    @KafkaListener(topics = "seller-stock-update", groupId = "stock-update")
    @Transactional
    ...
    @Transactional
    public void checkStock(Message<?> message) throws JsonProcessingException {

        List<OrdersProductDto> ordersProductDtoList = objectMapper.convertValue(message.getPayload(), new TypeReference<List<OrdersProductDto>>() {});
        ...
        for (OrdersProductDto ordersProductDto : ordersProductDtoList) {
            ...
            if (orderProductCount > stockCount) {
                ...
                stockCountMap.put(ordersProductDto.getProductId(), stockCount - orderProductCount);
